        ...
        + 'static,
{
    ...
    let prepare_for_admin_or_authorized_user_only = warp::header::<String>("authorization")
        .and(with_store.clone())
        .and(with_admin_auth_header.clone());

    // Receives parameters which were prepared by `prepare_for_admin_or_authorized_user_only` and
    // considers the request is eligible to be processed or not, checking the auth.
    // Why we separate `prepare_for_admin_or_authorized_user_only` and this function is that
    // we want to check whether the sender is eligible to access this path but at the same time,
    // we don't want to spawn any `Rejection`s at `prepare_for_admin_or_authorized_user_only`.
    // At the point of `prepare_for_admin_or_authorized_user_only`, there might be some other
    ...
    // This function needs parameters which can be prepared by `prepare_for_admin_or_authorized_user_only`.
    ...
    let prepare_for_authorized_user_only =
        warp::header::<String>("authorization").and(with_store.clone());

    // The same structure as `admin_or_authorized_user_only`.
    // This function needs parameters which can be prepared by `prepare_for_authorized_user_only`.
    let authorized_user_only = |path_username: Username, auth_string: String, store: S| {
        let auth: AuthToken = match AuthToken::from_str(&auth_string) {
            Ok(auth) => auth,
            Err(_) => return Either::A(err::<A, Rejection>(ApiError::account_not_found().into())),
        };
        Either::B(
            store
                .get_account_from_http_auth(auth.username(), auth.password())
    ...
    let get_account = warp::get2()
        ...
        .and(warp::path::end())
        .and(prepare_for_admin_or_authorized_user_only.clone())
    ...
    let post_payments = warp::post2()
        ...
        .and(warp::path("payments"))
        .and(warp::path::end())
        .and(prepare_for_authorized_user_only.clone())
